...
if ($filter_mapel) $where[] = "j.mapel='".$conn->real_escape_string($filter_mapel)."'";
if ($filter_bulan) $where[] = "MONTH(j.tanggal_upload)='".sprintf('%02d',(int)$filter_bulan)."'";
$where_sql = $where ? 'WHERE '.implode(' AND ', $where) : '';
$jadwal = $conn->query("SELECT j.*, k.nama_kelas FROM jadwal j LEFT JOIN kelas k ON j.id_kelas=k.id_kelas $where_sql ORDER BY j.tanggal_upload DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<div class="main">
    <div class="card card-section">
        <h2 class="section-title"><i class="fa-solid fa-calendar-alt"></i> Jadwal Kelas Anda</h2>
    ...
    <div class="table-responsive">
        <table class="table table-striped" border="1" cellpadding="6" style="border-collapse:collapse;">
            <thead>
                <tr><th>Tipe</th><th>File</th><th>Tanggal</th></tr>
            </thead>
            <tbody>
                <?php while($j=$jadwal->fetch_assoc()): ?>
                <tr>
                    <td><?=htmlspecialchars($j['tipe'])?></td>
                    <td><?php if($j['file_pdf']): ?><a href="../uploads/jadwal/<?=htmlspecialchars($j['file_pdf'])?>" target="_blank"><i class="fas fa-file-download"></i> File</a><?php endif; ?></td>
